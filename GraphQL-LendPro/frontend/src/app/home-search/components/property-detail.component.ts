    ...
    this.propertyService.searchProperties(searchFilters).subscribe({
      next: (results) => {
        ...
        const uniqueProperties = results.properties
          .filter((p: any) => p.id !== this.property!.id)
          .filter((property: any, index: number, array: any[]) => 
            array.findIndex((p: any) => p.id === property.id) === index
          );
        ...
        if (similarProperties.length < 3) {
          ...
          const remainingProperties = uniqueProperties.filter((p: any) => 
            !similarProperties.some((sp: any) => sp.id === p.id)
          );
